### Node Cache Stage
FROM node:16-alpine as node_cache_transactions_local
# Configure path
WORKDIR /src/
RUN chmod -R 777 /src/
...
COPY ./ .
COPY --chown=${N_USER}:${N_USER} --from=node_cache_transactions_local /src/node_modules ./node_modules
COPY --chown=${N_USER}:${N_USER} --from=node_cache_transactions_local /src/yarn.lock ./yarn.lock
